...
for _ in range(n):
    ...
    tu_dien[ma_nv] = {'Họ và tên': ho_ten, 'Năm sinh': nam_sinh, 'Lương': luong}
ma_nv_tim_kiem = input("Nhập mã nhân viên cần tìm kiếm: ")
if ma_nv_tim_kiem in tu_dien:
    print("Thông tin nhân viên:")
    print(f"Mã nhân viên: {ma_nv_tim_kiem}")
    for key, value in tu_dien[ma_nv_tim_kiem].items():
        print(f"{key}: {value}")
else:
...
if ma_nv_tang_luong in tu_dien:
    tu_dien[ma_nv_tang_luong]['Lương'] += muc_tang
    print(f"Đã tăng lương cho nhân viên có mã {ma_nv_tang_luong}.")
else:
    print("Không tìm thấy nhân viên có mã số này")
ma_nv_xoa = input("Nhập mã nhân viên cần xóa: ")
if ma_nv_xoa in tu_dien:
    del tu_dien[ma_nv_xoa]
    print(f"Đã xóa nhân viên có mã {ma_nv_xoa} khỏi danh sách.")
else:
    print("Không tìm thấy nhân viên có mã số này.")
tu_dien_sap_xep = dict(sorted(tu_dien.items(), key=lambda x: x[1]['Năm sinh'], reverse=True))
print("\nDanh sách nhân viên sau khi sắp xếp giảm dần theo năm sinh:")
for ma_nv, thong_tin in tu_dien_sap_xep.items():
